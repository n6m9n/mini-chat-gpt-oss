...
used by `generate.py`) and a rolling full-state `out/latest.pt` (model + optimizer
+ scaler + iter) for resuming.

### Resuming long runs (e.g. Kaggle's 12h session limit)
```bash
# first session
python train.py --device cuda --max_iters 20000
# later sessions: pick up exactly where it stopped
python train.py --device cuda --max_iters 20000 --resume
```
Pass the **same CLI flags** when resuming (block size / etc. must match the
checkpoint). Point data and outputs at a shared/writable dir via env vars:
```bash
DATA_DIR=/kaggle/working/data OUT_DIR=/kaggle/working/out_base \
